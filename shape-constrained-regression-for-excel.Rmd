...
predict_scar <- function(object, newdata, type = c("link", "response"), rule=1, ...){
...
```{r}
mod_scar <- fit_scar(y ~ x1 + x2, shape=c("in", "de"), data=train_data, family=binomial())
```
...
map_dfr(list(gam=preds_gam, mono_gam=preds_mono_gam, pseudo=preds_pseudo, scar=preds_scar), function(x){
  caTools::colAUC(x, test_data$y)
}) %>% 
  pivot_longer(everything(), names_to="model", values_to="ROC-AUC") %>% 
  knitr::kable()
...
pseudo_train_data_nls$y <- predict_scar(mod_scar, newdata=train_data, type="link")
...
map_dfr(list(gam=preds_gam, mono_gam=preds_mono_gam, pseudo=preds_pseudo,
             scar=preds_scar, pseudo_scar=preds_pseudo_scar), function(x){
  caTools::colAUC(x, test_data$y)
}) %>% 
  pivot_longer(everything(), names_to="model", values_to="ROC-AUC") %>% 
  knitr::kable()
